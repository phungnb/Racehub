...
**Kết luận chiến lược:** RaceHub = *UpRace (thử thách/đội VN)* + *Strava (xã hội)* + *Zwift/Sweatcoin (game hóa, kinh tế Xu)*. Điểm khác biệt nằm ở **Cheer có giá trị** và **cược 1-1 / đội**. Nhưng tất cả đều cần một nền móng mà mọi app trên đã có: **dữ liệu chạy tự động, đáng tin cậy.**
...
| **Thử thách** | adidas/UpRace: theo mùa, đội, BXH real-time, huy chương | 🟡 Tạo được, tiến độ cộng dồn km | Trang chi tiết + BXH + tham gia/rời + tất toán + huy chương → **Sprint 2** |
| **Bảng xếp hạng** | Strava Club weekly, UpRace đội | ❌ | View BXH tuần/tháng (cá nhân, CLB, khu vực) → **Sprint 2** |
| **Cấp độ & thành tích** | NRC badges theo mốc; Zwift mở khóa đồ | 🟡 Cấp độ đúng tài liệu | Huy hiệu tự động (km đầu tiên, 100 km, streak 7 ngày), PB 5K/10K/HM/FM → **Sprint 3** |
| **Kinh tế Xu** | Sweatcoin: trần ngày, chợ đổi quà | ✅ Sổ cái, trần ngày | Nhiệm vụ ngày/tuần, lịch sử ví (MH 25) → **Sprint 3** |
| **CLB** | Strava Club: BXH tuần, feed nhóm, sự kiện | ✅ Quản trị, quỹ | BXH tuần trong CLB, thử thách nội bộ → **Sprint 2** |
...
| Sprint | Mục tiêu | Hạng mục chính |
|---|---|---|
| **1. Dữ liệu tự động** *(đang làm)* | Bài chạy từ Garmin/COROS/Apple Watch qua Strava tự vào RaceHub, có thưởng và tính thử thách | Ingest RPC (idempotent, chống gian lận), nút Đồng bộ, backfill 30 ngày khi kết nối, webhook, refresh token, xử lý xóa/deauthorize, hiển thị nguồn và trạng thái |
| **2. Thử thách & BXH** | Vòng lặp cạnh tranh | Chi tiết thử thách, tham gia/rời, BXH real-time, BXH tuần CLB, tất toán + huy chương |
| **3. Xã hội & giữ chân** | Lý do quay lại mỗi ngày | Feed tự sinh, Like/Cheer, huy hiệu, PB, nhiệm vụ ngày, ví Xu, thông báo |
| **4. Native & doanh thu** | Trải nghiệm chạy chuẩn và kiếm tiền | App Expo (GPS nền, voice coach), Web Dashboard BTC, thanh toán, training plan |
