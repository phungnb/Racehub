...
> [`supabase/deploy/chay_tu_003700.sql`](../supabase/deploy/chay_tu_003700.sql) (gộp sẵn **003700 → 006300** + chạy lại 003500),
...
| `20261001004100_security_governance.sql` | **Bảo mật + lưu vết:** khóa ghi bảng cũ, bỏ quyền của khách với hàm quản trị, cố định search_path, việc định kỳ chỉ cho máy chủ; nhật ký quản trị + sổ cái **không sửa / xóa được** (admin vẫn toàn quyền, không cần người thứ hai duyệt); chốt thử thách **không cộng XP**; thưởng quỹ CLB tối đa 50% số dư và chỉ trao khi ≥ 3 người có kết quả | Cần file 4000; chạy lại 3500 |
...
| `20261001004300_admin_quests_promotions.sql` | **Admin tạo nhiệm vụ** (Quản trị → Nhiệm vụ): ngày / tuần / tháng / **sự kiện** theo khoảng ngày, chỉ số tổng km / số bài / bài dài nhất / điểm danh…, thưởng Xu, có thể chỉ cho VIP. **Khuyến mãi** (Quản trị → Khuyến mãi): tặng hàng loạt theo nhóm (tất cả, đang chạy, lâu không chạy, người mới, VIP, gói miễn phí, thành viên CLB, từ cấp, chọn tay) bằng Xu / lượt tạo / gói VIP; **mã khuyến mãi** nhập ở Ví; **đợt giảm giá gói / tặng thêm Xu khi nạp** áp thẳng vào đơn | Cần file 4200; chạy lại 3500 |
| `20261001004400_system_notice_client_errors.sql` | **Báo lỗi cho người dùng + thông báo hệ thống.** Admin đăng thông báo chung (Thông tin / Cảnh báo / **Bảo trì**, có giờ tự tắt) ở **Quản trị → Hệ thống**, hiện ở đầu app cho mọi người (kể cả màn đăng nhập). App tự gửi lỗi người dùng gặp (máy chủ lỗi, mất kết nối máy chủ, tính năng chưa cập nhật…) về mục **Lỗi người dùng gặp**, gộp theo mã lỗi — mã này cũng hiện trên màn lỗi của người dùng để họ báo lại. Chống spam 30 lần / giờ / người | Cần file 4300; chạy lại 3500 |
| `20261001004500_run_synced_notify.sql` | **"Bài chạy đã về"**: bài từ Strava / đồng hồ vừa được cộng thưởng → điện thoại nhận thông báo kèm km, pace, Xu, XP; bấm vào mở trang chủ và tự bật màn nhận thưởng. Bài nhập lại lịch sử (> 2 ngày) không báo | Cần file 4400; chạy lại 3500 |
| `20261001004600_quest_engine_v2.sql` | **Nhiệm vụ v2 + gợi ý cho admin.** Nhiệm vụ **bậc** (VD: chạy 3 / 5 / 10 km — chỉ trả tới bậc cao nhất), kỳ **Một lần** cho người mới / cột mốc, chỉ số mới: ngày chạy trong kỳ, bài cuối tuần, bài chạy sớm, thử thách hoàn thành, **km cả cộng đồng**. Thưởng thêm **vật phẩm / huy hiệu giới hạn / lượt tạo** (không XP). Trần Xu nhiệm vụ 5/ngày, 25/tuần; tối đa 3 nhiệm vụ ngày, 4 tuần, 5 tháng đang bật (chỉnh ở Quản trị → Nhiệm vụ). Admin có **gợi ý theo số liệu, dịp trong năm, thư viện mẫu** và **ước tính số người đạt + Xu chi ra** trước khi đăng | Cần file 4500; chạy lại 3500. Nếu đang bật quá số nhiệm vụ cho phép, tắt bớt nhiệm vụ cũ trước khi thêm mới |
| `20261001004700_shine_economy.sql` | **Ví Tỏa sáng.** Quà nhận được có chỗ tiêu: **Tỏa sáng tích lũy** (danh tiếng, không giảm) quyết định **khung phát sáng quanh ảnh đại diện** — Lấp lánh 500 · Rạng rỡ 2.000 · Chói sáng 10.000 · Huyền thoại 50.000; **Tỏa sáng khả dụng** đổi được lượt tạo thử thách ≤20 / ≤50 người, khiên giữ chuỗi, vật phẩm nhân vật chỉ có ở đây (≈ 30–35% giá trị Xu, không đổi ra Xu). Chống gian lận: chỉ tính người tặng ≥ 14 ngày tuổi + ≥ 3 bài chạy, mỗi người góp tối đa 300/tuần, đổi lượt tạo cần ≥ 5 người tặng. Lời cảm ơn miễn phí, huy hiệu 5/20/50 người hâm mộ, bộ sưu tập quà theo độ hiếm, ẩn tường quà, "Được tiếp sức nhiều nhất" trong thử thách. Chỉnh giá / luật ở **Quản trị → Quà tặng → Cửa hàng Tỏa sáng** | Cần file 4600; chạy lại 3500. Quà đã tặng trước đây được tính lại tự động |
| `20261001004800_design_layers.sql` | **Thiết kế BIB + Giấy chứng nhận theo lớp.** Mọi phần tử kéo thả / xoay / phóng to: chữ (40 font tiếng Việt nhúng sẵn, hiệu ứng viền, bóng, phát sáng, nổi khối, bôi dạ quang, nền khối, băng chéo), ảnh / logo / logo nhà tài trợ, mã QR (xác thực VĐV, trang giải, đơn vị tổ chức, **phí tham gia** lấy sẵn từ Quỹ CLB, đường link, ảnh QR tải lên), hình trang trí. 8 mẫu BIB + 4 mẫu chứng nhận (dọc 4:5 / ngang A4) có **bố cục tự động**. Số BIB in lên bỏ tiền tố chữ. Thiết kế BIB cũ tự chuyển đổi khi mở | Cần 002900 → 003200; chạy lại 3500. Deploy app **cùng lúc** với migration (bản app cũ lưu BIB sẽ được nhắc tải lại trang) |
| `20261001004900_challenge_honors.sql` | **Vinh danh thử thách (CLB Pro / VIP).** Tab "Vinh danh" trong thử thách: BTC chọn hạng mục (Top thành tích, Nhiều km, Chạy đều nhất, Chuỗi ngày liên tiếp, Bứt phá nhất, Được tiếp sức nhiều nhất, tối đa 5 giải tự đặt), thiết kế **ảnh nhóm** + **ảnh cá nhân** bằng trình kéo thả (10 nền vẽ sẵn, 4 khổ ảnh, khung ảnh runner có căn/phóng ảnh trong khung, chữ nhũ vàng). Chỉ **công bố sau khi kết thúc 24 giờ**; người được vinh danh nhận thông báo + huy hiệu "Được vinh danh", tự đổi ảnh hoặc **ẩn mình**. Kho ảnh `honor-media` | Cần 4800; chạy lại 3500 |
| `20261001005000_club_ownership_fix.sql` | **Sửa lỗi trao quyền Chủ nhiệm CLB** (lỗi "Không thực hiện được" — hàm cũ gán vai trò không tồn tại). Chủ nhiệm cũ thành Quản trị viên, người nhận phải là thành viên đã duyệt, báo cho Chủ nhiệm mới. Sửa luôn Chủ nhiệm rời CLB (tự trao cho Quản trị viên vào sớm nhất) | Chạy độc lập, **chạy ngay**; chạy lại 3500 |
