...
import { buildBundle, buildParts, PART_LIMIT } from '../../scripts/db-bundle.mjs'
...
describe('file gộp triển khai (supabase/deploy)', () => {
  ...
  it('bản chia nhỏ (supabase/deploy/phan): khớp migration hiện tại, mỗi phần ≤ ~90 KB, chạy lần lượt (2 lần) → đủ migration', async () => {
    const parts = buildParts('003700')
    for (const p of parts) {
      expect(fs.readFileSync(path.join(__dirname, '../../supabase/deploy/phan', p.name), 'utf8')).toBe(p.sql)
      if (p.files.length > 1) expect(Buffer.byteLength(p.sql)).toBeLessThan(PART_LIMIT + 8 * 1024)
    }
    const db = await createDb({ withMigrations: true, until: '20261001003600' })
    for (let round = 0; round < 2; round++) for (const p of parts) await db.exec(sanitize(p.sql))
    await db.exec(`
      insert into auth.users (id, email) values ('${ADMIN}', 'a@x.vn');
      insert into public.profiles (id, display_name, xu, xp, level, created_at) values ('${ADMIN}', 'Admin', 0, 0, 1, now()) on conflict do nothing;
      update public.profiles set role = 'SYSTEM_ADMIN' where id = '${ADMIN}';
