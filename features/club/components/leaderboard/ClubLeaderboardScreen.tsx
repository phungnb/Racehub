...
function MemberLeaderboard({ clubId }: { clubId: string }) {
  ...
  return (
    <div className="space-y-4 pb-20">
      <BoostDays clubId={clubId} isStaff={isStaff} />
    ...
    <div id="club-podium" className="grid scroll-mt-20 grid-cols-3 items-end gap-2 rounded-[var(--radius-card)] border border-border bg-surface px-3 pt-5">
      {order.map((r, i) => r ? (
        <div key={r.user_id} className="flex flex-col items-center text-center">
          ...
          <p className="font-mono tabular text-sm font-bold">{formatKm(r.distance_m + r.bonus_m)} km</p>
          {r.bonus_m > 0 && <p className="text-[11px] font-semibold text-coin">+{formatKm(r.bonus_m)} vàng</p>}
