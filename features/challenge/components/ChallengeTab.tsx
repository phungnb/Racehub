    ...
    <div className="space-y-4 text-white p-4">
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className="bg-gradient-to-r from-amber-500 to-brand-strong hover:opacity-90 text-slate-950 text-xs font-bold px-4 py-2.5 rounded-xl shadow-lg transition-all flex items-center space-x-1 cursor-pointer"
        >
          <span>⚡ Tạo Thử Thách</span>
        </button>
      </div>

      <div className="bg-surface-2 border border-[#3a3b3c] px-4 py-3 rounded-xl flex items-center justify-between text-xs">
        ...
        <span className="text-[11px] text-brand bg-brand/10 px-2.5 py-1 rounded-lg border border-brand/20 font-semibold">
      ...
      <div className="space-y-3 pt-2">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold text-brand uppercase tracking-wider">🎯 Danh Sách Thử Thách ({challenges.length})</h3>
        </div>

        {loading ? (
          <div className="text-center py-8 text-xs text-gray-500">Đang đồng bộ dữ liệu sổ cái...</div>
        ) : challenges.length === 0 ? (
          <div className="bg-surface border border-border rounded-2xl p-6 text-center text-xs text-gray-400">
            Chưa có thử thách nào được khởi tạo. Hãy bấm &quot;Tạo Thử Thách&quot; để khám phá ngay!
          </div>
        ) : (
          <div className="space-y-3">
            {challenges.map((item) => (
              <div key={item.id} className="bg-surface border border-border rounded-xl p-4 space-y-2 hover:border-amber-500/50 transition-all">
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-bold ${
                    item.challenge_type === 'TEAM' ? 'bg-brand/10 text-brand border border-brand/30' : 'bg-amber-500/10 text-amber-400 border border-amber-500/30'
                  }`}>
                ...
                <div className="flex items-center justify-between text-xs text-gray-400 pt-1 border-t border-border">
