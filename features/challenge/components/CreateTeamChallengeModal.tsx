...
export default function CreateTeamChallengeModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-[#18191a] border border-[#2f3031] w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden text-white">
        
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#2f3031] bg-[#242526]">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <input 
              ...
              className="w-full bg-[#242526] border border-[#3a3b3c] rounded-xl px-4 py-3 text-white focus:outline-none focus:border-amber-500 transition-colors"
            />
          </div>
                ...
                <div 
                  ...
                  className={`p-4 rounded-xl border cursor-pointer transition-all ${
                    gameMode === mode.id 
                      ...
                      : 'bg-[#242526] border-[#3a3b3c] text-gray-300 hover:border-gray-500'
                  }`}
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input 
                ...
                className="w-full bg-[#242526] border border-[#3a3b3c] rounded-xl px-4 py-2.5 text-white focus:outline-none focus:border-amber-500"
              />
            </div>
            <div>
          ...
          {/* Footer Buttons */}
          <div className="flex items-center justify-end space-x-3 pt-4 border-t border-[#2f3031]">
            <button 
              type="button" 
              onClick={onClose}
              className="px-5 py-2.5 rounded-xl text-sm font-semibold text-gray-400 hover:text-white hover:bg-[#242526] transition-all"
            >
              Hủy bỏ
            </button>
            <button 
              type="submit" 
              disabled={loading}
              className="px-6 py-2.5 rounded-xl text-sm font-bold bg-gradient-to-r from-amber-500 to-orange-600 text-white hover:opacity-95 shadow-lg shadow-amber-500/20 transition-all flex items-center space-x-2"
            >
