...
Chỉ lấy phần biểu tượng (bỏ chữ RACEHUB và khẩu hiệu vì quá nhỏ ở cỡ icon), tách khỏi nền, phóng to
...
def render(mask: Image.Image, bg: bool = True, glow: bool = True, color: tuple = WHITE) -> Image.Image:
    ...
    if glow:
        ...
        g = Image.new('RGBA', (N, N), (170, 240, 255, 0))
        g.putalpha(mask.filter(ImageFilter.GaussianBlur(N * 0.03)).point(lambda v: int(v * 0.45)))
...
def main():
    ...
    save(render(scaled(base, 0.9), bg=False, glow=False), 96, f'{V}-badge-96.png', rgb=False)
